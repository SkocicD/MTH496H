...
This uniqueness is an important property of Delaunay Triangulations, but it is not at all apparent.
This feature can be difficult to prove, but for now, just take it to be true.
An explanation of this will be provided in the section on Voronoi diagrams.
For now, we should show that a triangulation like this exists for any set of points, otherwise this definition wouldn't be very useful in an algorithm meant for any set of points.

Even before proving this, a few more results of will help better understand some properties of Delaunay triangulations.
I will first begin by proposing a way to compare triangulations to one another on the basis of the interior angles of their triangles.
...
We say that $T>T'$ if $A$ is lexiographically larger than $A'$.
...
Since these edges have a triangle on each side, they serve as a diagonal of a quadrilateral.
I will refer to this quadrilateral as the 'surrounding quadrilateral'.
...
Now, we can relate the definition of thin and thick edges to the empty circle definition of the Delaunay triangulation.
...
\begin{proof}
	Consider a convex quadrilateral $ABCD$ with a circle connecting $A, B,$ and $C$, with $D$ inside the circle, and bisect $ABCD$ with $\overline{AC}$.

	For one diagonal, label the angles Let this quadrilateral be
\end{proof}
...
\begin{theorem}
	A Delaunay Triangulation exists for any given set of points $S$.
\end{theorem}
\begin{proof}
